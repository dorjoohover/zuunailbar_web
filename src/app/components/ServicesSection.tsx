...
function ServiceCard({ item, index }: { item: { icon?: React.ElementType; title: string; id: number, price: string }; index: number }) {
  ...
  return (
    <div onClick={() => router.push(`/service?active=${item.id}`)}

 key={index} className={cn("justify-start gap-6 px-8 py-10 text-center border shadow-md bg-white/40 border-gray-200/50 backdrop-blur-3xl rounded-3xl col-center cursor-pointer hover:scale-105 duration-300", index === 7 && "z-20")}>
